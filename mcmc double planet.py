...
def log_likelihood(theta, t, y, yerr):
    P1, K1, e1, w1, P2, K2, e2, w2, v0 = theta
    # Calculate the model radial velocity curve
    model = v0 + K1 * np.sin(2*np.pi*t/P1 + w1) + K2 * np.sin(2*np.pi*t/P2 + w2) 
...
def log_prior(theta):
    ...
    if (1 < P1 < 1000 and 0 < K1 < 100 and 0 <= e1 < 1 and 0 <= w1 < 2*np.pi and
        1 < P2 < 1000 and 0 < K2 < 100 and 0 <= e2 < 1 and 0 <= w2 < 2*np.pi and -100 < v0 < 100):
        return 0.0
    return -np.inf
...
ndim, nwalkers = 9, 100
pos = np.array([154, 28, 0.16, np.pi/2, 885, 15, 0.4, 0, 2.21]) + 1e-4*np.random.randn(nwalkers, ndim)
...
labels = ['P1', 'K1', 'e1', 'w1', 'P2', 'K2', 'e2', 'w2', 'v0']
...
P1, K1, e1,w1, P2, K2, e2, w2,v0= np.median(samples, axis=0)
...
print(f"v0= {v0:.2f} ± {np.std(samples[:, 8]):.2f} ")

t_fit = np.linspace(min(t), max(t), 1000)
params_fit = np.median(samples, axis=0)
model = v0 + K1 * np.sin(2*np.pi*t_fit/P1 + w1) + K2 * np.sin(2*np.pi*t_fit/P2 + w2) 
...
fig = corner.corner(samples, labels=labels, quantiles=[0.95, 0.98, 0.99], show_titles=True, title_kwargs={"fontsize": 12})
plt.show()